...
public class PromptController
{
    ...
    public Flux<String> chat6(String question){
        return  deepseekChatClient.prompt()
                .system("你是一个资深Java开发程序员，你只能回答java相关的问题，其他问题回复，请问java问题")
                .user( question)
                .stream()
                .content();
    }
    ...
    /**
     * http://localhost:8005/prompt/chat7?question=葫芦娃
     * @param question
     * @return
     */
    @GetMapping("/prompt/chat7")
    public Flux<ChatResponse> chat7(String question){
    ...
    public Flux<String> chat3(String question)
    {
        ...
        Prompt prompt = new Prompt(userMessage, systemMessage);
//将ChatResponse转化为只显示文字
        return deepseekChatModel.stream(prompt)
                .map(response -> response.getResults().get(0).getOutput().getText());

    }

    @GetMapping("/prompt/chat8")
    public Flux<String> chat8(String question){
        SystemMessage systemMessage = new SystemMessage("你是一个讲故事的助手"+"每个故事控制在600字以内且以HTML格式返回");
        UserMessage userMessage = new UserMessage(question);
        Prompt prompt = new Prompt(userMessage, systemMessage);
    ...
    @GetMapping("/prompt/chat9")
    public String chat9(String question){
